...
| `nested_cv_jcp2/` | not in the paper | cXGBoost with the settings reselected inside each training fold over the 27-point grid: per-fold selections and outer errors (`<case>.json`, `<case>_errors.csv`), the comparison with Table 2 (`report.json`, `summary.json`) |
